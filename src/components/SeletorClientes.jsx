...
export default function SeletorClientes({ 
  ...
  const { clientes, loading: clientesLoading, isLoaded } = useCache()
  ...
  return (
    <div className="bg-white p-6 rounded-lg shadow-sm border border-gray-200">
      <div className="flex justify-between items-center mb-4">
        ...
        {clientesSelecionados.length > 0 && (
          <button
            ...
            className="text-sm font-semibold text-red-700 hover:text-red-900 bg-red-50 hover:bg-red-100 px-2 py-1 rounded transition-colors"
          >
            Limpar seleção
          </button>
        )}
      ...
      {clientesSelecionados.length > 0 && (
        <div className="mb-4">
          <p className="text-sm font-semibold text-gray-900 mb-2">Selecionados:</p>
          <div className="flex flex-wrap gap-2">
            {clientesSelecionados.map((cliente) => (
              <span
                key={cliente}
                className="inline-flex items-center px-3 py-1 rounded-full text-xs font-semibold bg-blue-100 text-blue-900 border border-blue-200"
              >
                {cliente}
                <button
      ...
      {clientes.length === 0 && (
        <div className="text-center py-6 text-gray-600">
          ...
          <p className="text-sm mt-1 font-medium">Importe dados primeiro na página Lista de Clientes</p>
        </div>
      )}
    </div>
  )
